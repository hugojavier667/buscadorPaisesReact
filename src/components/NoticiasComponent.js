...
class NoticiasComponent extends Component {
  ...
  next() {
    ...
    const nextIndex = this.state.activeIndex === 10 - 1 ? 0 : this.state.activeIndex + 1;
    this.setState({ activeIndex: nextIndex });
  }

  previous() {
    if (this.animating) return;
    const nextIndex = this.state.activeIndex === 0 ? 10 - 1 : this.state.activeIndex - 1;
    this.setState({ activeIndex: nextIndex });
  }
  ...
  render() {
    ...
    return (
      ...
      >
        <CarouselIndicators items={noticias} activeIndex={activeIndex} onClickHandler={this.goToIndex} />
        {slides}
